...
## product Array
The products object is a fundamental component that manages product data in the template system.
...
| `featured_image.src` | string | Main product image URL | null |
| `images[].src` | string | Additional image URLs | [] |
| `variants` | array | Product variants | [] |
| `variants[].id` | string | Variant identifier | "" |
| `variants[].title` | string | Variant title | "" |
| `variants[].available_for_sale` | boolean | Stock availability | false |
| `variants[].price` | string | Variant price | "0.00" |
| `variants[].compare_at_price` | string | Original price | null |
| `variants[].image.src` | string | Variant image URL | null |
| `variants[].selected_options` | array | Selected variant options | [] |
| `variants[].selected_options[].name` | string | Option name | "" |
| `variants[].selected_options[].value` | string | Option value | "" |
| `options` | array | Product options | [] |
| `options[].name` | string | Option name | "" |
| `options[].option_values` | array | Available option values | [] |
| `show_compare_price` | boolean | Show compare at price | false |
| `first_variant` | object | First available variant | null |
...
**NOTE:** The `new`, `trending`, and `bestseller` translations are used to display badge text on product cards. These badges help highlight product status or popularity to customers. The badges will automatically appear on products based on their tags or other criteria set in the template settings.
...
| `layout` | string | Section layout type ('horizontal_grid', etc.) | "horizontal_grid" |
| `location` | string | Section placement location | "product_page" |
| `position` | number | Section position order | 0 |
| `title` | string | Section heading text | "" |
| `template` | string | Template identifier | "default" |
| `extra` | object | Additional section configuration | {} |
| `discount_config` | object | Discount settings and configuration | null |
| `show_discount_label` | boolean | Whether to display discount labels | false |
| `translations` | object | Section-specific translations | {} |
| `template_v3` | boolean | Whether using template version 3 | false |
| `section_serve_id` | string | Unique serve identifier | Generated UUID |
| `page_type` | string | Type of page section appears on | "product" |
...
| Key | Type | Description | Default Value |
|-----|------|-------------|---------------|
| `clientWidth` | number | Current viewport width in pixels | 1024 |
| `clientHeight` | number | Current viewport height in pixels | 768 |
| `devicePixelRatio` | number | Device pixel ratio for image optimization | 1 |
...
| Key | Type | Description | Default Value |
|-----|------|-------------|---------------|
| `config.integrations_enabled` | array | List of enabled integration slugs | [] |
| `config.product_review_app` | string | Selected product review app name | null |
| `config.swatch_app` | string | Selected swatch app name | null |
