...
1. **Tags**: Custom Liquid tags for template functionality
   ```liquid
   {% product_review_app_integration product_id: product.id %}
   ```
...
Templates are loaded through our JavaScript embed:

```html
<div id="glood-section-12345" data-section-type="recommendations">
  <!-- Template will be loaded here -->
</div>

<script>
  window.GloodAI.loadSection({
    container: '#glood-section-12345',
    template: 'horizontal_grid'
  });
</script>
```
