...
**Description:**  
Finds an object in an array based on a specified property and value.

**Parameters:**
- `key` (String, required): Property to match
...
**Description:**  
Translates a key using the translations object with an optional default value.

**Parameters:**
- `translations` (Object, required): Translations object
- `default` (String, optional): Default value if translation not found

**Returns:**  
String - Translated text or default value

**Example:**
```liquid
{% assign oos_text = 'oos_text' | t: translations, 'Out of stock' %}
{{ 'discount_label_text' | t: translations, '' }}
```
...
|-------|-------|----------|
| `Undefined filter` | Filter name misspelled or not available | Check filter name and ensure template has access to all filters |
| `Missing required parameter` | Required parameter not provided | Ensure all required parameters are passed to the filter |
| `Invalid input type` | Input value type doesn't match filter requirements | Convert input to correct type before passing to filter |
| `Object not found` | Referenced object doesn't exist in context | Verify object exists in template context before using filter |
