...
{% assign section_discount_str = section.discount_config.value | section_discount: section.discount_config.type, section.discount_config.widget_message, glood.localization.language.locale, glood.shop.money_format, template.settings.intl_options %}
{% assign image_dim = client_info.client_width | image_dimensions: template.settings.breakpoints %}
{% assign hide_atc = template.settings.product_card.variant_selector_type == 'swatch_with_atc' or template.settings.product_card.hide_atc %}
...
>
  ...
  {% if section.layout == 'horizontal_grid' %}
    <ul class="_gai-grid">
      {% for product in products %}
        {% assign product_url = root_url | product_url: product.handle, glood.product.id, section.layout, product.id %}
        ...
        {% capture swatch_component %} 
          {% assign swatch_enabled = template.settings.product_card.variant_selector_type == 'swatch' or template.settings.product_card.variant_selector_type == 'swatch_with_atc' %}
          {% if swatch_enabled and product.variants.size > 1 %}
                  ...
                  <div class="_gai-swt-ch-opt">
                    ...
                    <div class="_gai-swtch-opt">
                      {% for value in found_option.option_values %}
                        <span
                          class="_gai-swtch-btn {% if swatch.type != "color" %} _gai-swtch-sz-btn {% else %} _gai-swtch-clr-btn {% endif %}"
                          data-option-name="{{ found_option.name }}"
                          data-option-value="{{ value }}"
                          {% if template.settings.product_card.variant_selector_type == 'swatch_with_atc' %}
                            data-selected="false"
                          {% else %}
                            data-selected="{{ selected_option.value == value | to_boolean }}"
                          {% endif %}
                          data-oos="{{ available_variants_count == 0 | to_boolean }}"
                          style="{% if swatch.type == "color" %} background: {{ value | color_value: template.settings.color_mapping }} {% endif %}"
                        >
                          {% if swatch.type != "color" %}{{ value }}{% endif %}
                        </span>
                      {% endfor %}
                    </div>
                  </div>
          ...
          {% if size > 1 %}
            ...
            >
              {% for variant in product.variants %}
                ...
                >
                  {{ variant.title }} ({{ variant.price | format_money: glood.shop.money_format, template.settings.intl_options }})
                </option>
              {% endfor %}
            </select>
          {% endif %}
        ...
        <li class="_gai-grid-item">
          <div
            class="_gai-prod-card swiper-slide"
          ...
          >
            ...
            <div class="_gai-lbls-grp">
              ...
              {% for label in template.settings.labels %}
                ...
                {% endif %}
              {% endfor %}
              {% assign product_discount = product | product_discount: section.discount_config, translations.discount_label_text, glood.shop.money_format, template.settings.intl_options %}
            ...
            <div class="_gai-frame">
              ...
              >
                <img
                  ...
                  {% endif %}
                  src="{{ image_src | optimize_image: image_dim.width, image_dim.height }}"
                  alt="{{ product.title }}"
                >
                {% if product.images[1].src %}
                  <img
                    class="_gai-prod-img _gai-primary-prod-img _gai-sec-img"
                    src="{{ product.images[1].src | optimize_image: image_dim.width, image_dim.height }}"
                    alt="{{ product.title }}"
                  >
                {% endif %}
              ...
              <div class="_gai-full-width ">
                ...
                <div class="_gai-prod-price-wrp" data-compare-price="{{ product.show_compare_price }}">
                  {% if template.settings.product_card.price_compare_at_mode == 'before' %}
                    <span
                      class="_gai-prod-prc _gai-prod-prc--original"
                    >
                    </span>
                  {% endif %}
                  <span
                    class="_gai-prod-prc _gai-prod-prc--compare"
                  >
                ...
                {% assign save_text = product | save_amount: section.discount_config, translations.discount_save_text, glood.shop.money_format, template.settings.intl_options %}
  ...
  {% elsif section.layout == 'carousel' %}
    <div class="_gai-crz-cnt swiper">
      <div class="swiper-wrapper gai-crz-wrap">
        {% for product in products %}
          {% assign first_available_variant = -1 %}
          {% assign available_variants_count = 0 %}
          {% for variant in product.variants %}
      ...
      <div class="_gai-crz-scrbr">
        <div class="_gai-crz-scrbr-drag">_</div>
      </div>
              ...
              <div class="_gai-prod-dtls">
                <div class="_gai-full-width">
                  ...
                  {% if template.settings.product_card.vendor_position == 'below' %}
                    <div class="_gai-prod-vndr">{{ product.vendor }}</div>
                  {% endif %}
                  {% assign save_text = product | save_amount: section.discount_config, translations.fbt_save_text, glood.shop.money_format, template.settings.intl_options %}
                  {% if save_text.length > 0 %}
                    <div class="_gai-pd-save-t">{{ save_text }}</div>
                  {% endif %}
                  <div class="_gai-prod-price-wrp" data-compare-price="{{ product.show_compare_price }}">
                    ...
                    <div class="_gai-swtch-wrp">
                      {% for swatch in template.settings.swatches %}
                        ...
                        {% if found_option != false %}
                          ...
                          <div class="_gai-swt-ch-opt">
                            <label>{{ translations[translation_key] | default: found_option.name }}</label>
                            {% assign selected_option = first_available_variant.selected_options
                              | find: 'name', found_option.name
                            %}
                            <div class="_gai-swtch-opt">
                              {% for value in found_option.option_values %}
                                <span
        ...
        <div class="_gai-amz-details">
          ...
          <div class="_gai-amz-prc">
            <div class="_gai-amz-price">
              ...
              {% endif %}
              {% if template.settings.amazon_bought_together.price_compare_at_mode == 'before' %}
                <div
                  class="_gai-bndl-cmp-prc-value _gai-prod-prc _gai-prod-prc--original"
                  data-visible="true"
                ></div>
              {% endif %}
              <div
              ...
              ></div>
              {% if template.settings.amazon_bought_together.price_compare_at_mode == 'after' %}
                <div
                  class="_gai-bndl-cmp-prc-value _gai-prod-prc _gai-prod-prc--original"
                  data-visible="true"
                ></div>
